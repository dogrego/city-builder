...
{
	for (; currentPtr < endPtr && std::isspace(static_cast<unsigned char>(*currentPtr)); ++currentPtr)
	{
		...
		case From2Char('f', '\t'): // f (<pi>[/<ti>][/<ni>])3+
		{
			...
			while (!faceVertT.empty())
			{
				...
				if (texEndOffs > texStartOffs)
					std::from_chars(faceVertT.data() + texStartOffs, faceVertT.data() + texEndOffs, idxVert.vt);
				if (idxVert.vt)
					idxVert.vt--;
